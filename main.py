...
def print_help():
    ...
    print ' -O ... use configuration to generate output image name'
    print ' -c <columns>'
...
try:
    opts, args = getopt.getopt(sys.argv[1:], 'hi:o:Oc:s:t:T:f:CF:B:S:vna')
...
if generate_output_name:
    input_file_parts = input_file.split('.')
    output_file = '.'.join(input_file_parts[:-1])

    output_file += '-%sc' % columns
    output_file += '-%sx' % scale
    output_file += '-%0.f' % threshold
    output_file += '-%ssp' % font_size
    if colorize:
        output_file += '-%s' % colorize
        output_file += '-%s' % background
        output_file += '-%s' % foreground

    output_file += '.' + input_file_parts[-1]

if verbose:
    print 'input: %s' % input_file
    print 'output is generated by config: %s' % generate_output_name
